...
**Parser: incomplete pattern matching.** `parser.js` line 1097 has a `TODO: Handle more complex parameter expressions` — pattern matching in function parameters is not fully implemented.
...
7. **Tail-call optimization.** Functional languages benefit greatly from this. The current implementation would stack-overflow on deep recursion.
...
**Missing: error message guide.** No documentation of common runtime errors and what causes them. Given that errors currently lack source locations, this would be especially valuable.
...
- **Native `.test.rix` runner** — `bun rix/tools/rix.js test` discovers and runs RiX-native test files. 4 test files currently.
- **REPL** — `bun rix/tools/rix.js` provides an interactive REPL with meta-commands (`.help`, `.vars`, `.reset`, `.ast`, `.tokens`).
...
| Evaluator implementation | **B+** | Functional and correct, but large files, no source locations in errors |
...
| Error messages | **C+** | Informative text but no source locations — hard to debug |
